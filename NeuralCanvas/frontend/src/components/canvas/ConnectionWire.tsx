...
import { BLOCK_REGISTRY, type BlockType } from "@/lib/blockRegistry";
...
const COLORS = {
  valid: { stroke: "#22c55e", glow: "#22c55e40", bg: "rgba(22,101,52,0.85)", border: "rgba(34,197,94,0.5)", text: "#bbf7d0" },
  error: { stroke: "#ef4444", glow: "#ef444440", bg: "rgba(69,10,10,0.90)", border: "rgba(239,68,68,0.5)", text: "#fecaca" },
  unknown: { stroke: "#6b7280", glow: "#6b728020", bg: "rgba(31,41,55,0.85)", border: "rgba(107,114,128,0.4)", text: "#9ca3af" },
} as const;
...
// ---------------------------------------------------------------------------
// Flowing-dot animation via SVG marker + dash animation
// ---------------------------------------------------------------------------

/** Unique SVG id per edge — keeps markers from clashing. */
function dotMarkerId(edgeId: string) {
  return `flow-dot-${edgeId}`;
}
...
function ConnectionWireComponent({
  ...
  const markerId = dotMarkerId(id);

  return (
    <>
      {/* ── SVG defs: flowing dot marker ── */}
      <defs>
        <marker
          id={markerId}
          markerWidth="6"
        ...
        >
          <circle cx="3" cy="3" r="2" fill={colors.stroke} opacity="0.9" />
        </marker>
      </defs>

      {/* ── Glow layer (wider, blurred, behind the main stroke) ── */}
      <path
        d={edgePath}
        fill="none"
        stroke={colors.stroke}
        strokeWidth={selected ? 6 : 4}
        strokeOpacity={hovered ? 0.2 : 0.08}
        style={{ filter: "blur(3px)", transition: "stroke-opacity 0.3s ease" }}
      />
      ...
      <BaseEdge
        ...
        style={{
          ...
          strokeDasharray: "6 4",
          strokeLinecap: "round",
          animation: "connectionFlowDash 0.6s linear infinite",
